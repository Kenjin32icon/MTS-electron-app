...
// Modified updateNavigationVisibility function (kept here for general UI updates, but auth-specific parts are in auth.js)
...
// Utility function to switch tabs (kept here for general use, but auth.js has its own for internal use)
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Navigation links (kept here as they are part of general UI)
    const navLinks = document.querySelectorAll('.nav-links a');
    ...
    initAuth(authButton, signInUpModal, closeSignInUpModal, loginForm, signupForm, tabButtons, firstLoginModal, closeFirstLoginModal, firstLoginForm);

    // Check authentication immediately after auth.js is initialized
    ...
    if (!currentUser || currentUser.success === false) {
        ...
        // Show login modal (handled by initAuth now, but keeping this check for clarity)
        if (signInUpModal) {
            signInUpModal.style.display = 'block';
            // switchTab('signup'); // Default to sign-up tab - handled by initAuth
        }
        // The rest of the login/signup form event listeners are now in auth.js
    ...
    // Dark Mode Toggle (kept here as it's general UI)
